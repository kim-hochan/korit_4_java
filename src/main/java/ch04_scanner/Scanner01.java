...
//        System.out.println("이름 " + name);
        /*
            ...
            그 다음 nextLine()을 사용했다고 가정하면 생기는 문제가 있다.

            next어쩌고 다음에 어려분이 엔터를 때려버리면
            그 다음 줄에 있는 nextLine()이 enter키가 입력된걸 보고
            아무런 값도 집어넣지 않고 바로 넘겨버리는 현상이 생긴다.

            해당 문제 상황을 해결하기 위한 방법 :
            nextLine()이 미리 enter 키를 받아들여서 name에 아무런 값도
            입력하지 않고(대입하지 않고) 십고 지나가는 상황이라고 할 수 있다.

            -> enter키를 한 번 받아주는 애가 있으면 됨
...
//        System.out.println(univ);
        ...
        // 차이점은 .md 파일에 정리하시기 바랍니다.
        //배리어 관련 정도는 블로그에 올라가도 괜찮다.
